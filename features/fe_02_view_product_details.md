...
- [ ] Pass `App`'s `setView` method through `ProductList` via props so that when a user clicks on a `ProductListItem`, `App`'s `view` state gets updated to `{ name: 'details', params: { id: product.id } }`
